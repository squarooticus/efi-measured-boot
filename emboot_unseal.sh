...
if [ "$CRYPTTAB_TRIED" = 0 ]; then
    outcome() {
        ...
        [ -z "${CRYPTTAB_SOURCE}" ] || log -t tpm -l $log_prio 'backing device: %s' "${CRYPTTAB_SOURCE}"
        [ -z "${CRYPTTAB_NAME}" ] || log -t tpm -l $log_prio 'mapped name: %s' "${CRYPTTAB_NAME}"
        log -t tpm -l $log_prio 'token ID: %s' "$tid"
        log -t tpm -l $log_prio 'kernel release: %s' "$krel"
    }
    ...
    if would_log -t tpm -l $LL_DEBUG; then set -x; fi

    tmpdir=$(setup_tmp_dir)

    verbose_do -t tpm -l $LL_INFO eval 'read_pcrs >$tmpdir/current_pcrs.txt'
    verbose_do -t tpm -l $LL_INFO eval 'read_counter "$tmpdir"/current_counter'
    ...
    for tid in $(list_luks_token_ids "$CRYPTTAB_SOURCE" "$krel"); do
        ...
        log_info -t tpm 'counter: current=%d expects<=%d' "0x$(xxd -p -c9999 <$tmpdir/current_counter)" "0x$(xxd -p -c9999 <$tmpdir/counter)"
        verbose_do -t tpm -l $LL_INFO eval 'diff_pcrs "$tmpdir"/pcrs "$tmpdir"/current_pcrs.txt | sed -e "s/^/  /"'
    done
